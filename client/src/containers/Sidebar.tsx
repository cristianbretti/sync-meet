...
const userDisplay = (user: DBUser, owner_id: number, your_id: number) => (
    <div key={user.id}>
        <div className="px-2 text-center">
            <h4>
                {user.name}
                {showPersonIcon(user, your_id)}
                {showOwnerIcon(user, owner_id)}
                {showRefreshIcon(user, your_id)}
            </h4>
        </div>
    </div>
)
...
const refreshUserToken = (googleResponse: any) => {
    ...
    const group_str_id = window.location.href.substr(
    ...
    return (
        ...
        >
            ...
            <div className="text-center absolute pin-b pin-r flex ">
                ...
                <button
                    onClick={() => leaveGroup(your_id, owner_id, group_str_id)}
